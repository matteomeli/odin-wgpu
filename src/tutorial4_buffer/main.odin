...
init :: proc "c" () {
    ...
    on_device :: proc "c" (status: wgpu.RequestDeviceStatus, device: wgpu.Device, message: wgpu.StringView, userdata1: rawptr, userdata2: rawptr) {
        ...
        io_utils.read_entire_file_to_string("assets/shaders/shader.wgsl", proc(shader_code: string, ok: bool) {
            if !ok {
                return
            }
            ...
            state.render_pipeline = wgpu.DeviceCreateRenderPipeline(state.device, &wgpu.RenderPipelineDescriptor {
